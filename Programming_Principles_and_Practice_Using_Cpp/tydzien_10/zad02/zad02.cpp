...
try{
   ...
   //mfile.m.size(); //poczatkowo 5, po modyfikacji 4 poniewaz udalo wyeliminowaæ sie odczytywanie fragmentu:
                            //----
                            //----
                            //jako kolejna wiadomosc (poczatek i koniec wiadomosci)
   cout << "mfile.m.size() = 5 przed zmiana, a po zmianie = 4" << endl;
}
catch (exception& e) {
    cerr << e.what() << '\n'; // write error message
    return 1;
